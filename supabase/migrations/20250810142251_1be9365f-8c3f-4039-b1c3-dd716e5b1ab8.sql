-- Idempotent setup after previous failure: ensure types/tables first, then functions/policies
...
-- Enums (idempotent)
create type if not exists public.app_role as enum ('donor','recipient','admin');
create type if not exists public.blood_type as enum ('A+','A-','B+','B-','AB+','AB-','O+','O-');
create type if not exists public.request_urgency as enum ('low','medium','high','critical');
create type if not exists public.request_status as enum ('open','matched','fulfilled','cancelled','expired');
create type if not exists public.match_status as enum ('pending','accepted','declined','cancelled','completed');
create type if not exists public.push_platform as enum ('ios','android','web');
create type if not exists public.push_provider as enum ('fcm','onesignal','apns','webpush');
...
-- Validation function for requests
...
-- Triggers (drop if exist then recreate)
-- profiles
...
-- RLS Policies (drop-n-recreate for idempotency)
-- profiles
